...
import {
  ...
  Mention,
  withMentions
} from "./mention_plugin/mentoin_element";
...
const RichText = (props: any) => {
  ...
  const withs: any = [withReact, withHistory, withMentions];
  ...
  const [onChange, onKeyDown, Menu]: any = useMention(
    editor,
    /^@(\w+)$/,
    CHARACTERS,
    insertMention
  ...
  const [onChange_E, onKeyDown_E, Menu_E]: any = useMention(
    editor,
    /^\/(\w+)$/,
    Object.keys(components_elements),
    insertElement
  );
...
const Element = (props: any) => {
  ...
  const render = (fragment && fragment["element"](props)) ||
    elements[element.type] || <p {...attributes}>{children}</p>;
  return render;
};
